...
std::vector<glm::vec2> cellMidpoints(const int columns, const int rows, const glm::vec4 & viewport)
{
    if (0 == rows || 0 == columns)
        return std::vector<glm::vec2>();

    const auto w = (viewport[2] - viewport[0]) / columns;
    const auto h = (viewport[3] - viewport[1]) / rows;
    const auto o = glm::vec2(viewport);

    auto midpoints = std::vector<glm::vec2>(columns * rows);

    for (auto r = 0; r < rows; ++r)
        for (auto c = 0; c < columns; ++c)
            midpoints[r * columns + c] = glm::vec2((0.5f + c) * w, (0.5f + r) * h) + o;

    return midpoints;
}

glm::vec2 cellScale(const int columns, const int rows
    , const glm::vec4 & viewport, const int width, const int height
    , const float padding)
{
    if (0 == rows || 0 == columns)
        return glm::vec2(0.f);
    ...
    const auto size = glm::vec2(w, h) - glm::vec2(padding / aspect, padding);
    const auto scale = std::min(size.x, size.y / aspect);

    return glm::vec2(scale, scale * aspect);
}
